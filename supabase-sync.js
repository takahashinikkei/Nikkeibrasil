...
async function createAccessRequest(x,file){
 if(!sb)throw new Error('Banco de dados indisponível.');
 const row={username:x.username,password_hash:x.passwordHash,name:x.name,surname:x.surname,address:x.address,address_number:x.addressNumber,neighborhood:x.neighborhood,state:x.state,city:x.city,phone:x.phone,phone2:x.phone2,phone_alt:x.phoneAlt,emergency_contact_name:x.emergencyContactName||null,cpf:x.cpf,rg:x.rg,function_role:x.functionRole||null,vehicle_type:x.vehicleType,vehicle_subtype:x.vehicleSubtype,body_type:x.bodyType,vehicle_configuration:x.vehicleConfiguration,plate_cavalo:x.plateCavalo,plate_carreta:x.plateCarreta,plate_carreta2:x.plateCarreta2,status:'pending'};
