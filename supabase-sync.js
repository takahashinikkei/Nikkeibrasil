...
async function loadDrivers(){
 ...
 const list=(data||[]).map(r=>({id:r.id,name:r.name||'',surname:r.surname||'',address:r.address||'',addressNumber:r.address_number||'',neighborhood:r.neighborhood||'',state:r.state||'',city:r.city||'',phone:r.phone||'',phone2:r.phone2||'',phoneAlt:r.phone_alt||'',emergencyContactName:r.emergency_contact_name||'',cpf:r.cpf||'',rg:r.rg||'',plate:r.plate||'',vehicleType:r.vehicle_type||'',vehicleSubtype:r.vehicle_subtype||'',bodyType:r.body_type||'',vehicleConfiguration:r.vehicle_configuration||'',plateVehicle:r.plate||'',plateCavalo:r.plate_cavalo||'',plateCarreta:r.plate_carreta||'',plateCarreta2:r.plate_carreta2||'',cnhPath:r.cnh_path||'',cnhName:r.cnh_name||'',cnhMime:r.cnh_mime||''}));
...
async function loginAccess(username,passwordHash){if(!sb)return null;const {data,error}=await sb.from('access_requests').select('*').ilike('username',username).eq('password_hash',passwordHash).eq('status','approved').maybeSingle();if(error)throw error;return data?accessRequestFromRow(data):null}
...
async function createAccessRequest(x,file){
 if(!sb)throw new Error('Banco de dados indisponível.');
 const row={username:x.username,password_hash:x.passwordHash,name:x.name,surname:x.surname,birth_date:x.birthDate||null,address:x.address,address_number:x.addressNumber,neighborhood:x.neighborhood,state:x.state,city:x.city,phone:x.phone,phone2:x.phone2,phone_alt:x.phoneAlt,emergency_contact_name:x.emergencyContactName||null,cpf:x.cpf,rg:x.rg,function_role:x.functionRole||null,vehicle_type:x.vehicleType,vehicle_subtype:x.vehicleSubtype,body_type:x.bodyType,vehicle_configuration:x.vehicleConfiguration,plate:x.plateVehicle||null,plate_cavalo:x.plateCavalo,plate_carreta:x.plateCarreta,plate_carreta2:x.plateCarreta2,status:'pending'};
...
window.nikkeiLogin=loginAccess;window.nikkeiCreateAccessRequest=createAccessRequest;window.nikkeiGetAccessRequest=getAccessRequest;window.nikkeiLoadAccessRequests=loadAccessRequests;window.nikkeiUpdateAccessRequest=updateAccessRequest;
window.nikkeiGetVehiclePlates=async function(kind){if(!sb)return[];const {data,error}=await sb.from('vehicles').select('plate,vehicle_type,vehicle_subtype,title').not('plate','is',null);if(error)throw error;const map={VAN:{vehicle_type:'CM',vehicle_subtype:'VAN'},'3/4':{vehicle_type:'CM',vehicle_subtype:'3/4'},TOCO:{vehicle_type:'CM',vehicle_subtype:'TOCO'},TRUCK:{vehicle_type:'CM',vehicle_subtype:'TRUCK'},BITRUCK:{vehicle_type:'CM',vehicle_subtype:'BITRUCK'},CAV4X2:{vehicle_type:'CV',vehicle_subtype:'TOCO'},CAV6X2:{vehicle_type:'CV',vehicle_subtype:'6X2'},CAV6X4:{vehicle_type:'CV',vehicle_subtype:'6X4'},CARRETA:{vehicle_type:'CR'}};const f=map[kind];if(!f)return[];const seen=new Set();return data.filter(x=>x.vehicle_type===f.vehicle_type&&(!f.vehicle_subtype||x.vehicle_subtype===f.vehicle_subtype)).map(x=>({plate:String(x.plate).trim(),title:x.title||''})).filter(x=>x.plate&&!seen.has(x.plate)&&seen.add(x.plate));}function patchStorage(){
