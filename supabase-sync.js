...
window.nikkeiGetVehiclePlates=async function(kind){if(!sb)return[];const {data,error}=await sb.from('vehicles').select('plate,vehicle_type,vehicle_subtype,title').eq('status','stock').not('plate','is',null);if(error)throw error;const map={VAN:{vehicle_type:'CM',vehicle_subtype:'VAN'},'3/4':{vehicle_type:'CM',vehicle_subtype:'3/4'},TOCO:{vehicle_type:'CM',vehicle_subtype:'TOCO'},TRUCK:{vehicle_type:'CM',vehicle_subtype:'TRUCK'},BITRUCK:{vehicle_type:'CM',vehicle_subtype:'BITRUCK'},CAV4X2:{vehicle_type:'CV',vehicle_subtype:'TOCO'},CAV6X2:{vehicle_type:'CV',vehicle_subtype:'6X2'},CAV6X4:{vehicle_type:'CV',vehicle_subtype:'6X4'}};const f=map[kind];if(!f)return[];return data.filter(x=>x.vehicle_type===f.vehicle_type&&x.vehicle_subtype===f.vehicle_subtype).map(x=>({plate:x.plate,title:x.title||''})).filter(x=>x.plate);};
