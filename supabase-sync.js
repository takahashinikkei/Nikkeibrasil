...
function vehiclePayload(x,status){
 ...
 reference_code:x.referenceCode||x.refCode||null,plate:x.plate||null,km:x.km!==''&&x.km!=null?Number(x.km)||0:null,color:x.color||null,vehicle_type:x.vehicleType||null,vehicle_subtype:x.vehicleSubtype||null,body_type:x.bodyType||null,pallets:x.pallets?Number(x.pallets):null,body_height:x.bodyHeight?Number(x.bodyHeight):null,body_width:x.bodyWidth?Number(x.bodyWidth):null,body_length:x.bodyLength?Number(x.bodyLength):null,pc:x.pc||null,
...
function vehicleFromRow(r){
 ...
 priceValue:r.price_value,referenceCode:r.reference_code||'',plate:r.plate||'',km:r.km,color:r.color||'',vehicleType:r.vehicle_type||'',vehicleSubtype:r.vehicle_subtype||'',bodyType:r.body_type||'',pallets:r.pallets??'',bodyHeight:r.body_height??'',bodyWidth:r.body_width??'',bodyLength:r.body_length??'',pc:r.pc||'',purchaseDate:r.purchase_date||'',
